...
Included relationships can be limited and sorted with `resource:relationship` syntax:

```http
GET /song/song_123?include=albums,albums.tracks&limit[albums:tracks]=10&sort[albums:tracks]=name
```

Root list and relationship endpoints support `limit`, `offset`, and `sort`. Sorting supports `name`, `dateAdded`, and minus-prefixed descending variants. Song and album result sets also support `releaseDate`.
